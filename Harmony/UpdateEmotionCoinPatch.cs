...
namespace BigDLL4221.Harmony
{
    ...
    public class UpdateEmotionCoinPatch
    {
        ...
        private static Predicate<BattleUnitModel> Match => x =>
            ...
            BattleObjectManager.instance.GetAliveList(x.faction).Any(y =>
                y.passiveDetail.PassiveList.Exists(z => z is PassiveAbility_NoEmotionAndEgoFloor_DLL4221)) ||
            ModParameters.StageOptions.Any(b => b.Key == Singleton<StageController>.Instance.GetStageModel().ClassInfo
                .id.packageId && b.Value.Any(c =>
                c.StageId == Singleton<StageController>.Instance.GetStageModel().ClassInfo.id.id &&
                c.BannedEmotionLevel));
